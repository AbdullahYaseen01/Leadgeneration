"""
Web frontend for Lead Dataset Builder.
Run: flask --app app run  (or python app.py)
Deploy: Vercel (serverless) — uses sync mode and /tmp; set GOOGLE_PLACES_API_KEY in env.
"""
...
app = Flask(__name__)
# In-memory job store (not used on Vercel — serverless is stateless)
jobs: dict = {}
# Limit for web requests (optimized for 1000 leads when self-hosted)
MAX_LEADS_WEB = 1000
...
def _run_job(
    ...
    # Optional: cap run time when self-hosted (e.g. MAX_RUN_SECONDS=1500 for ~25 min)
    max_time = None
    try:
...
def _get_api_key():
    """Get Google API key from config file or env (env only on Vercel)."""
    if VERCEL:
...
def api_collect():
    ...
    if VERCEL:
        # Serverless: run synchronously with time limit; return CSV in response (no job store)
        try:
            leads = run_collection_for_cities_niches(
                api_key=api_key,
                cities=cities,
